      ...
      <v-col>
        ...
        <v-card elevation="0">
          <v-card-title class="d-flex align-center pe-2">
            <v-btn class="mb-3 text-capitalize" color="primary" prepend-icon="mdi-plus"
              @click="openCreateDialog()">Create</v-btn>
            <v-spacer></v-spacer>
          ...
          <v-divider></v-divider>
          <v-data-table density="compact" :search="search" :items="studentList" :headers="headers" :loading="loading"
            items-per-page="20">
            ...
            <template v-slot:[`item.actions`]="{ item }">
              <v-tooltip text="View Profile" location="top">
                <template v-slot:activator="{ props }">
                  <v-btn size="medium" :to="`/students/${item.documentId}`" class="mr-1" variant="text" v-bind="props"
    ...
    <v-dialog max-width="800" v-model="dialog" scrollable>
      <v-card>
        ...
        <v-card-text class="mx-5">
          <v-form v-model="valid" ref="createStudentForm" lazy-validation>
            ...
            <v-row dense>
              ...
              <v-col cols="12" md="8" sm="6">
                ...
                <!-- <v-select
                  ...
                  variant="outlined"
                ></v-select> -->
                <v-text-field v-if="selectedBA" color="primary" label="Major" v-model="major" variant="outlined">
            ...
            <v-row dense>
              ...
              <v-col cols="12" md="4" sm="6">
                <v-text-field color="primary" label="Middle name" v-model="middlename" variant="outlined"
                  persistent-hint @input="middlename = middlename.toUpperCase()" required></v-text-field>
              </v-col>
            </v-row>
            <v-row dense>
              <v-col>
        ...
        <v-card-actions>
          ...
          <v-btn color="green" text="Save" variant="flat" width="140" :loading="loadingCreateStudent"
            @click="createStudent()" prepend-icon="mdi-content-save"></v-btn>
          <v-spacer></v-spacer>
        </v-card-actions>
      </v-card>
    </v-dialog>
